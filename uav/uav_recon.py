...
def _yolo_visualizer_thread(model, latest_img, current_uav_pose, shutdown_flag, active_tracks, final_objects,
                            conf_thresh, target_fps, img_size):
    """
        ...
        if current_time - last_inference_time >= inference_interval:
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    new_detections.append(
                        {'class_id': cls_id, 'class_name': model.names[cls_id], 'cx': cx, 'cy': cy, 'x1': x1, 'y1': y1,
                         'x2': x2, 'y2': y2})
            DIST_THRESH = 80
            TIME_THRESH = 3.0
            for det in new_detections:
                ...
                for track in active_tracks:
                    if track['class_id'] == det['class_id']:
                        ...
                        if dist < DIST_THRESH and (current_time - track['last_seen_time']) < TIME_THRESH:
                            ...
                            track['x1'], track['y1'], track['x2'], track['y2'] = det['x1'], det['y1'], det['x2'], det[
                                'y2']
                            matched_active = True
                            break
                ...
                if current_uav_pose[0]:
                    uav_pose_dict = {"x": current_uav_pose[0].pose.position.x, "y": current_uav_pose[0].pose.position.y,
                                     "z": current_uav_pose[0].pose.position.z}
            ...
            cv2.putText(annotated_frame, f"Unique Targets: {len(final_objects)} (Inf: {target_fps}fps)", (20, 40),
...
def _run_uav_waypoint_flight(waypoints, vel_lin, vel_z):
    ...
    def goal_cb(msg):
        nonlocal target_pose, has_temp_goal; target_pose = msg; has_temp_goal = True

    def sigint_handler(signum, frame):
        nonlocal shutdown_flag; shutdown_flag = True; sys.exit(0)
        ...
        while not rospy.is_shutdown() and not shutdown_flag:
            ...
            if has_temp_goal:
                ...
                if is_waiting:
                    ...
                    tx, ty, tz = waypoints[waypoint_index]
            else:
            ...
            if dist < POSITION_TOLERANCE:
                if has_temp_goal:
                    ...
                    is_waiting = True; wait_start_time = time.time()
            elif dist > 0:
...
def convert_pcd_to_2d_map(
    ...
    result = {
        ...
        "person": {"x": person_pose[0], "y": person_pose[1],
                   "confidence": round(person_conf, 4)} if person_pose else None,
...
def _run_batch_slicing(pcd_path, output_dir, ref_map_path, res, z_start, z_step, z_thick, runs, flip_lr, k_size,
                       min_area, dilate_iter, sim_thresh):
    ...
    for i in range(runs):
        ...
        if ref_img is not None:
            ...
            if curr_sim > best_sim:
                ...
                best_yaml = {"image": f"{name}.pgm", "resolution": res, "origin": [float(min_x), float(min_y), 0.0],
...
def _calculate_similarity(img1, img2):
    ...
    hist_sim = cv2.compareHist(cv2.calcHist([img1], [0], None, [256], [0, 256]),
                               cv2.calcHist([img2], [0], None, [256], [0, 256]), cv2.HISTCMP_CORREL)
...
def _run_optimize_and_detect(best_pgm_path, best_yaml_data, output_pgm, output_yaml, output_dir):
    ...
    res = best_yaml_data["resolution"];
    ox, oy = best_yaml_data["origin"][0], best_yaml_data["origin"][1]
    ...
    if coords is not None:
        ...
        img_crop = img_rot; (x_off, y_off) = (0, 0)
    ...
    if robot_pose:
        print(f"🤖 小车 MAP: x={robot_pose[0]:.3f}, y={robot_pose[1]:.3f} (Conf: {robot_conf:.2f})")
    else:
        print("❌ 未找到小车")
...
if __name__ == "__main__":
    ...
    WAYPOINTS = [
        [0.0, 0.0, 2.5],[4.5, 0.0, 2.5],[4.5, 4.0, 2.5],[-0.5, 4.0, 2.5],
        [-0.5, 8.0, 2.5],[4.5, 8.0, 2.5],[2.0, 8.0, 2.5],[2.0, 4.0, 2.5]
    ]
    # 记录起飞点位置，用于后续返航
    home_pose_msg = [None]


    def get_initial_pose(msg):
        home_pose_msg[0] = msg
